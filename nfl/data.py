...
def getWeek(seasonStart, gameDateStr):
    ...
    :param seasonStart: base serial date
    :param gameDateStr: number of adjustments to apply (integer)
    :returns: week of the season
    """
...
def readGamesSingleSeason(dataRoot, season):
    ...
    :param dataRoot: Path to the data file with all the spreads
    ...
    for ss in seasons:
        ...
        for ii, tt in enumerate(teams):
            ...
            dfTeam['wonGame'] = ((dfTeam['Visitor Score'] < dfTeam['Home Score']) & dfTeam['homeGame']) | (
                (dfTeam['Visitor Score'] > dfTeam['Home Score']) & (dfTeam['homeGame'] == False))
